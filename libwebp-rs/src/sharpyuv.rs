            ...
            loop {
                {
                    ...
                    let r: i32 = (best_uv[off.wrapping_add(0i32.wrapping_mul(uv_w)) as usize]
                        as i32)
                        .wrapping_add(W);
                    let g: i32 = (best_uv[off.wrapping_add(1i32.wrapping_mul(uv_w)) as usize]
                        as i32)
                        .wrapping_add(W);
                    let b: i32 = (best_uv[off.wrapping_add(2i32.wrapping_mul(uv_w)) as usize]
                        as i32)
                        .wrapping_add(W);
                    ...
                    if yuv_bit_depth <= 8i32 {
                        ...
                        crate::scylla_glue::scylla_u16_of_u8_mut(y_ptr)[i as usize] =
                            clip(y, yuv_max)
                    }
                };
                if {
            ...
            loop {
                {
                    ...
                    if yuv_bit_depth <= 8i32 {
                        ...
                        crate::scylla_glue::scylla_u16_of_u8_mut(u_ptr)[i as usize] =
                            clip(u, yuv_max);
                        crate::scylla_glue::scylla_u16_of_u8_mut(v_ptr)[i as usize] =
                            clip(v, yuv_max)
                    }
                };
                if {
...
pub fn DoSharpArgbToYuv(
    ...
    let mut tmp_buffer: Box<[u16]> = vec![
        0u16;
        tmp_buffer_size
            .wrapping_add(best_y_base_size)
            .wrapping_add(target_y_base_size)
            .wrapping_add(best_rgb_y_size)
            .wrapping_add(
                best_uv_base_size
                    .wrapping_add(target_uv_base_size)
                    .wrapping_add(best_rgb_uv_size)
            ) as usize
    ]
    .into_boxed_slice();
    ...
    let target_y_base: (&mut [u16], &mut [u16]) = (best_y_base.1).split_at_mut(
        tmp_buffer_size.wrapping_add(best_y_base_size) as usize - tmp_buffer_size as usize,
    );
    let best_rgb_y: (&mut [u16], &mut [u16]) = (target_y_base.1).split_at_mut(
        tmp_buffer_size
            .wrapping_add(best_y_base_size)
            .wrapping_add(target_y_base_size) as usize
            - tmp_buffer_size.wrapping_add(best_y_base_size) as usize,
    );
    let best_uv_base1: (&mut [u16], &mut [u16]) = (best_rgb_y.1).split_at_mut(
        tmp_buffer_size
            .wrapping_add(best_y_base_size)
            .wrapping_add(target_y_base_size)
            .wrapping_add(best_rgb_y_size) as usize
            - tmp_buffer_size
                .wrapping_add(best_y_base_size)
                .wrapping_add(target_y_base_size) as usize,
    );
    let target_uv_base1: (&mut [u16], &mut [u16]) = (best_uv_base1.1).split_at_mut(
        tmp_buffer_size
            .wrapping_add(best_y_base_size)
            .wrapping_add(target_y_base_size)
            .wrapping_add(best_rgb_y_size)
            .wrapping_add(best_uv_base_size) as usize
            - tmp_buffer_size
                .wrapping_add(best_y_base_size)
                .wrapping_add(target_y_base_size)
                .wrapping_add(best_rgb_y_size) as usize,
    );
    let best_rgb_uv1: (&mut [u16], &mut [u16]) = (target_uv_base1.1).split_at_mut(
        tmp_buffer_size
            .wrapping_add(best_y_base_size)
            .wrapping_add(target_y_base_size)
            .wrapping_add(best_rgb_y_size)
            .wrapping_add(best_uv_base_size)
            .wrapping_add(target_uv_base_size) as usize
            - tmp_buffer_size
                .wrapping_add(best_y_base_size)
                .wrapping_add(target_y_base_size)
                .wrapping_add(best_rgb_y_size)
        ...
        while j < height {
            {
                ...
                if is_last_row == 0i32 {
                    ...
                    (src2.1[0usize..3i32.wrapping_mul(w) as usize])
                        .copy_from_slice(&src2.0[0usize..3i32.wrapping_mul(w) as usize])
                };
                StoreGray(
                    src2.0,
                    &mut target_y_base.0[best_y_ofs.wrapping_add(0usize)..],
                    w,
                );
                StoreGray(
                    src2.1,
                    &mut target_y_base.0[best_y_ofs.wrapping_add(w as usize)..],
                    w,
                );
                UpdateW(
                    src2.0,
                    &mut best_rgb_y.0[target_y_ofs..],
                    w,
                    y_bit_depth,
                    transfer_type,
                );
                ...
                ((&mut best_uv_base[best_uv_ofs..])[0usize..3i32.wrapping_mul(uv_w) as usize])
                    .copy_from_slice(
                        &(&target_uv_base[target_uv_ofs..])
                            [0usize..3i32.wrapping_mul(uv_w) as usize],
                    );
                ...
                loop {
                    {
                            ...
                            (src1.1).split_at_mut(3i32.wrapping_mul(w) as usize);
                        {
                            let next_uv_ofs: usize =
                                cur_uv_ofs.wrapping_add(if j < h.wrapping_sub(2i32) {
                        ...
                        diff_y_sum = diff_y_sum.wrapping_add(SharpYuvUpdateY(
                        ...
                        target_y_ofs = target_y_ofs.wrapping_add(2i32.wrapping_mul(w) as usize);
                        target_uv_ofs =
                            target_uv_ofs.wrapping_add(3i32.wrapping_mul(uv_w) as usize);
                        j = j.wrapping_add(2i32)
                    };
                    if j >= h {
                        break;
...
#[inline]
pub fn Filter2(A: i32, B: i32, W0: i32, bit_depth: i32) -> u16 {
    let v0: i32 = A
        .wrapping_mul(3i32)
        .wrapping_add(B)
        .wrapping_add(2i32)
        .wrapping_shr(2u32);
    clip_bit_depth(v0.wrapping_add(W0), bit_depth)
}
...
pub fn ImportOneRow(
    ...
    let step: i32 = if rgb_bit_depth > 8i32 {
        rgb_step.wrapping_div(2i32)
    } else {
        rgb_step
    };
    let mut i: i32 = 0i32;
    ...
    loop {
        {
            ...
            if rgb_bit_depth == 8i32 {
                ...
                dst[i.wrapping_add(0i32.wrapping_mul(w)) as usize] = Shift(
                    crate::scylla_glue::scylla_u16_of_u8(r_ptr)[off as usize] as i32,
                    shift,
                ) as u16;
                dst[i.wrapping_add(1i32.wrapping_mul(w)) as usize] = Shift(
                    crate::scylla_glue::scylla_u16_of_u8(g_ptr)[off as usize] as i32,
                    shift,
                ) as u16;
                dst[i.wrapping_add(2i32.wrapping_mul(w)) as usize] = Shift(
                    crate::scylla_glue::scylla_u16_of_u8(b_ptr)[off as usize] as i32,
                    shift,
                ) as u16
            }
        };
    ...
    if pic_width & 1i32 != 0i32 {
        dst[pic_width.wrapping_add(0i32.wrapping_mul(w)) as usize] = dst[pic_width
            .wrapping_add(0i32.wrapping_mul(w))
            .wrapping_sub(1i32)
            as usize];
        dst[pic_width.wrapping_add(1i32.wrapping_mul(w)) as usize] = dst[pic_width
            .wrapping_add(1i32.wrapping_mul(w))
            .wrapping_sub(1i32)
            as usize];
        dst[pic_width.wrapping_add(2i32.wrapping_mul(w)) as usize] = dst[pic_width
            .wrapping_add(2i32.wrapping_mul(w))
            .wrapping_sub(1i32)
            as usize]
    }
}
...
pub fn InterpolateTwoRows(
    ...
    {
        out1[0usize] = Filter2(
            cur_uv[0usize] as i32,
            prev_uv[0usize] as i32,
            best_y[0usize] as i32,
            bit_depth,
        );
        out2[0usize] = Filter2(
            cur_uv[0usize] as i32,
            next_uv[0usize] as i32,
            best_y[w as usize] as i32,
            bit_depth,
        );
        SharpYuvFilterRow(
            cur_uv,
        ...
        if w & 1i32 == 0i32 {
            out1[w.wrapping_sub(1i32) as usize] = Filter2(
            ...
            out2[w.wrapping_sub(1i32) as usize] = Filter2(
                cur_uv[uv_w.wrapping_sub(1i32) as usize] as i32,
...
pub fn RGBToGray(r: i64, g: i64, b: i64) -> i32 {
    let luma: i64 = 13933i64
        .wrapping_mul(r)
        .wrapping_add(46871i64.wrapping_mul(g))
        .wrapping_add(4732i64.wrapping_mul(b))
        .wrapping_add(kYuvHalf as i64);
    luma.wrapping_shr(16u32) as i32
}

#[inline]
pub fn RGBToYUVComponent(r: i32, g: i32, b: i32, coeffs: [i32; 4], sfix: i32) -> i32 {
    let srounder: i32 = 1i32.wrapping_shl(16i32.wrapping_add(sfix).wrapping_sub(1i32) as u32);
    let luma: i32 = (coeffs[0usize])
        .wrapping_mul(r)
        .wrapping_add((coeffs[1usize]).wrapping_mul(g))
        .wrapping_add((coeffs[2usize]).wrapping_mul(b))
        .wrapping_add(coeffs[3usize])
        .wrapping_add(srounder);
    luma.wrapping_shr(16i32.wrapping_add(sfix) as u32)
}
...
pub fn SharpYuvConvert(
    ...
    let options: SharpYuvOptions = SharpYuvOptions {
        yuv_matrix,
...
pub fn SharpYuvConvertWithOptions(
    ...
    if yuv_bit_depth != 8i32 && yuv_bit_depth != 10i32 && yuv_bit_depth != 12i32 {
        return 0i32;
    };
    if rgb_bit_depth > 8i32
    ...
    if yuv_bit_depth > 8i32
        && (y_stride.wrapping_rem(2i32) != 0i32
            || u_stride.wrapping_rem(2i32) != 0i32
    ...
    if rgb_bit_depth == yuv_bit_depth {
        (std::slice::from_mut::<SharpYuvConversionMatrix>(&mut scaled_matrix)[0usize..1usize])
            .copy_from_slice(&yuv_matrix[0usize..1usize])
    } else {
        let mut i: i32 = 0i32;
        while i < 3i32 {
            {
                scaled_matrix.rgb_to_y[i as usize] = ((yuv_matrix[0usize]).rgb_to_y[i as usize])
                    .wrapping_mul(yuv_max)
                    .wrapping_add(rgb_round)
                    .wrapping_div(rgb_max);
                scaled_matrix.rgb_to_u[i as usize] = ((yuv_matrix[0usize]).rgb_to_u[i as usize])
                    .wrapping_mul(yuv_max)
                    .wrapping_add(rgb_round)
                    .wrapping_div(rgb_max);
                scaled_matrix.rgb_to_v[i as usize] = ((yuv_matrix[0usize]).rgb_to_v[i as usize])
                    .wrapping_mul(yuv_max)
                    .wrapping_add(rgb_round)
                    .wrapping_div(rgb_max)
            };
...
pub fn SharpYuvOptionsInitInternal<'a>(
    ...
    options[0usize] = SharpYuvOptions {
        yuv_matrix,
        transfer_type: SharpYuvTransferFunctionType::kSharpYuvTransferFunctionSrgb,
    };
    1i32
...
pub fn SharpYuvComputeConversionMatrix(
    ...
    let denom: f32 = 1i32
        .wrapping_shl((yuv_color_space[0usize]).bit_depth as u32)
...
pub fn SharpYuvGetConversionMatrix<'a>(
...
pub fn ToFixed16(f: f32) -> i32 {
    crate::math::floor((f * 1i32.wrapping_shl(16u32) as f32 + 0.5f32) as f64) as i32
}

pub const kRec601FullMatrix: SharpYuvConversionMatrix = SharpYuvConversionMatrix {
    rgb_to_y: [19595i32, 38470i32, 7471i32, 0i32],
    rgb_to_u: [
        0i32.wrapping_sub(11058i32),
        0i32.wrapping_sub(21710i32),
        32768i32,
        128i32.wrapping_shl(16u32),
    ],
    rgb_to_v: [
        32768i32,
        0i32.wrapping_sub(27439i32),
        0i32.wrapping_sub(5329i32),
        128i32.wrapping_shl(16u32),
    ],
};

pub const kRec601LimitedMatrix: SharpYuvConversionMatrix = SharpYuvConversionMatrix {
    rgb_to_y: [16829i32, 33039i32, 6416i32, 16i32.wrapping_shl(16u32)],
    rgb_to_u: [
        0i32.wrapping_sub(9714i32),
        0i32.wrapping_sub(19071i32),
        28784i32,
        128i32.wrapping_shl(16u32),
    ],
    rgb_to_v: [
        28784i32,
        0i32.wrapping_sub(24103i32),
        0i32.wrapping_sub(4681i32),
        128i32.wrapping_shl(16u32),
    ],
};

pub const kRec709FullMatrix: SharpYuvConversionMatrix = SharpYuvConversionMatrix {
    rgb_to_y: [13933i32, 46871i32, 4732i32, 0i32],
    rgb_to_u: [
        0i32.wrapping_sub(7509i32),
        0i32.wrapping_sub(25259i32),
        32768i32,
        128i32.wrapping_shl(16u32),
    ],
    rgb_to_v: [
        32768i32,
        0i32.wrapping_sub(29763i32),
        0i32.wrapping_sub(3005i32),
        128i32.wrapping_shl(16u32),
    ],
};

pub const kRec709LimitedMatrix: SharpYuvConversionMatrix = SharpYuvConversionMatrix {
    rgb_to_y: [11966i32, 40254i32, 4064i32, 16i32.wrapping_shl(16u32)],
    rgb_to_u: [
        0i32.wrapping_sub(6596i32),
        0i32.wrapping_sub(22189i32),
        28784i32,
        128i32.wrapping_shl(16u32),
    ],
    rgb_to_v: [
        28784i32,
        0i32.wrapping_sub(26145i32),
        0i32.wrapping_sub(2639i32),
        128i32.wrapping_shl(16u32),
    ],
};

pub const kWebpMatrix: SharpYuvConversionMatrix = SharpYuvConversionMatrix {
    rgb_to_y: [16839i32, 33059i32, 6420i32, 16i32.wrapping_shl(16u32)],
    rgb_to_u: [
        0i32.wrapping_sub(9719i32),
        0i32.wrapping_sub(19081i32),
        28800i32,
        128i32.wrapping_shl(16u32),
    ],
    rgb_to_v: [
        28800i32,
        0i32.wrapping_sub(24116i32),
        0i32.wrapping_sub(4684i32),
        128i32.wrapping_shl(16u32),
    ],
};
...
pub fn SharpYuvFilterRow_C(
    ...
    while i < len {
        {
            let v0: i32 = (A[0usize] as i32)
                .wrapping_mul(9i32)
                .wrapping_add((A[1usize] as i32).wrapping_mul(3i32))
                .wrapping_add((B[0usize] as i32).wrapping_mul(3i32))
                .wrapping_add(B[1usize] as i32)
                .wrapping_add(8i32)
                .wrapping_shr(4u32);
            let v1: i32 = (A[1usize] as i32)
                .wrapping_mul(9i32)
                .wrapping_add((A[0usize] as i32).wrapping_mul(3i32))
                .wrapping_add((B[1usize] as i32).wrapping_mul(3i32))
                .wrapping_add(B[0usize] as i32)
                .wrapping_add(8i32)
                .wrapping_shr(4u32);
            out[2i32.wrapping_mul(i).wrapping_add(0i32) as usize] = clip(
                (best_y[2i32.wrapping_mul(i).wrapping_add(0i32) as usize] as i32).wrapping_add(v0),
                max_y,
            );
            out[2i32.wrapping_mul(i).wrapping_add(1i32) as usize] = clip(
...
pub fn SharpYuvUpdateY(
    ...
    SharpYuvUpdateY_C(src, r#ref, dst, len, bit_depth)
}

pub fn SharpYuvUpdateY_C(
    r#ref: &[u16],
    src: &[u16],
    dst: &mut [u16],
    len: i32,
...
#[inline]
pub fn FixedPointInterpolation(
    v: i32,
    tab: &[u32],
    ...
    let v0: u32 = Shift(
        tab[tab_pos.wrapping_add(0u32) as usize] as i32,
        tab_value_shift,
    ) as u32;
    let v1: u32 = Shift(
        tab[tab_pos.wrapping_add(1u32) as usize] as i32,
        tab_value_shift,
    ) as u32;
    let v2: u32 = v1.wrapping_sub(v0).wrapping_mul(x);
    let half: i32 = if tab_pos_shift_right > 0i32 {
        1i32.wrapping_shl(tab_pos_shift_right.wrapping_sub(1i32) as u32)
    } else {
        0i32
    };
    let result: u32 = v0.wrapping_add(
        v2.wrapping_add(half as u32)
            .wrapping_shr(tab_pos_shift_right as u32),
    );
    result
}
...
pub fn FromLinear709(linear: f32) -> f32 {
    if linear < 0.0f32 {
        ...
        return 1.099_296_8_f32 * Powf(linear, 0.45_f32) - 0.099_296_82_f32;
    };
    1.0f32
}
...
pub fn FromLinearBt1361(linear: f32) -> f32 {
    if linear < 0f32 - 0.25f32 {
        ...
        return (0f32 - 0.274_824_2_f32) * Powf((0f32 - 4.0f32) * linear, 0.45_f32)
            + 0.024_824_206_f32;
    } else if linear < 0.018_053_97_f32 {
        return linear * 4.5f32;
    } else if linear < 1.0f32 {
...
pub fn FromLinearHlg(mut linear: f32) -> f32 {
    linear = Powf(linear, 1.0f32 / 1.2_f32);
    if linear < 0.0f32 {
        return 0.0f32;
    } else if linear <= 1.0f32 / 12.0f32 {
        return crate::math::sqrtf(3.0f32 * linear);
    };
    0.178_832_77_f32 * crate::math::logf(12.0f32 * linear - 0.284_668_92_f32) + 0.559_910_7_f32
}

pub fn FromLinearIec61966(linear: f32) -> f32 {
    if linear <= 0f32 - 0.018_053_97_f32 {
        return (0f32 - 1.099_296_8_f32) * Powf(0f32 - linear, 0.45_f32) + 0.099_296_82_f32;
    } else if linear < 0.018_053_97_f32 {
        return linear * 4.5f32;
    };
    1.099_296_8_f32 * Powf(linear, 0.45_f32) - 0.099_296_82_f32
}
...
pub fn FromLinearSmpte240(linear: f32) -> f32 {
    if linear < 0.0f32 {
        return 0.0f32;
    } else if linear < 0.022_821_585_f32 {
        return linear * 4.0f32;
    } else if linear < 1.0f32 {
        return 1.111_572_1_f32 * Powf(linear, 0.45_f32) - 0.111_572_2_f32;
    };
    1.0f32
}
...
pub fn FromLinearSmpte428(linear: f32) -> f32 {
    Powf(
        0.916_555_3_f32 * if linear > 0.0f32 { linear } else { 0.0f32 },
        1.0f32 / 2.6_f32,
    )
}
...
pub fn Log10f(x: f32) -> f32 {
    crate::math::log10(x as f64) as f32
}

#[inline]
pub fn Powf(base: f32, exp: f32) -> f32 {
    crate::math::pow(base as f64, exp as f64) as f32
}

#[inline]
pub fn Roundf(x: f32) -> f32 {
    if x < 0f32 {
        crate::math::ceil((x - 0.5f32) as f64) as f32
    } else {
        crate::math::floor((x + 0.5f32) as f64) as f32
    }
}
...
pub fn SharpYuvGammaToLinear(
    ...
    match transfer_type {
        SharpYuvTransferFunctionType::kSharpYuvTransferFunctionBt709 => {
            linear = ToLinear709(v_float)
        }
        SharpYuvTransferFunctionType::kSharpYuvTransferFunctionBt601 => {
            linear = ToLinear709(v_float)
        }
    ...
    Roundf(linear * 1i32.wrapping_shl(16u32).wrapping_sub(1i32) as f32) as u32
}
...
pub fn SharpYuvLinearToGamma(
    ...
    match transfer_type {
        ...
        SharpYuvTransferFunctionType::kSharpYuvTransferFunctionHlg => {
            linear = FromLinearHlg(v_float)
        }
        _ => linear = 0f32,
    };
    Roundf(linear * 1i32.wrapping_shl(bit_depth as u32).wrapping_sub(1i32) as f32) as u16
}

#[inline]
pub fn Shift(v: i32, shift: i32) -> i32 {
    if shift >= 0i32 {
...
pub fn ToLinear709(gamma: f32) -> f32 {
    if gamma < 0.0f32 {
        ...
        return Powf(
            (gamma + 0.099_296_82_f32) / 1.099_296_8_f32,
            1.0f32 / 0.45_f32,
        );
    };
    1.0f32
...
pub fn ToLinearBt1361(gamma: f32) -> f32 {
    if gamma < 0f32 - 0.25f32 {
        ...
        return Powf(
            (gamma - 0.024_824_206_f32) / (0f32 - 0.274_824_2_f32),
            1.0f32 / 0.45_f32,
        ) / (0f32 - 4.0f32);
    } else if gamma < 4.5f32 * 0.018_053_97_f32 {
        return gamma / 4.5f32;
...
pub fn ToLinearHlg(gamma: f32) -> f32 {
    if gamma < 0.0f32 {
        ...
        return Powf(gamma * gamma * (1.0f32 / 3.0f32), 1.2_f32);
    };
    Powf(
        (crate::math::expf((gamma - 0.559_910_7_f32) / 0.178_832_77_f32) + 0.284_668_92_f32)
            / 12.0f32,
        1.2_f32,
    )
}
...
pub fn ToLinearIec61966(gamma: f32) -> f32 {
    if gamma <= (0f32 - 4.5f32) * 0.018_053_97_f32 {
        return Powf(
            (0f32 - gamma + 0.099_296_82_f32) / (0f32 - 1.099_296_8_f32),
            1.0f32 / 0.45_f32,
        );
    } else if gamma < 4.5f32 * 0.018_053_97_f32 {
        return gamma / 4.5f32;
...
pub fn ToLinearLog100(gamma: f32) -> f32 {
    let mid_interval: f32 = 0.01_f32 / 2.0f32;
    if gamma <= 0.0f32 {
        mid_interval
    } else {
        Powf(
            10.0f32,
            2.0f32 * (if gamma < 1.0f32 { gamma } else { 1.0f32 } - 1.0f32),
        )
    }
}

pub fn ToLinearLog100Sqrt10(gamma: f32) -> f32 {
    let mid_interval: f32 = 0.003_162_277_6_f32 / 2.0f32;
    if gamma <= 0.0f32 {
        mid_interval
    } else {
        Powf(
            10.0f32,
            2.5f32 * (if gamma < 1.0f32 { gamma } else { 1.0f32 } - 1.0f32),
        )
    }
}
...
pub fn ToLinearPq(gamma: f32) -> f32 {
    if gamma > 0.0f32 {
        ...
        let num: f32 = if pow_gamma - 107.0f32 / 128.0f32 > 0.0f32 {
            pow_gamma - 107.0f32 / 128.0f32
        } else {
            0.0f32
        };
        let den: f32 =
            if 2413.0f32 / 128.0f32 - 2392.0f32 / 128.0f32 * pow_gamma > 1.175_494_4e-38_f32 {
...
pub fn ToLinearSmpte240(gamma: f32) -> f32 {
    if gamma < 0.0f32 {
        return 0.0f32;
    } else if gamma < 4.0f32 * 0.022_821_585_f32 {
        return gamma / 4.0f32;
    } else if gamma < 1.0f32 {
        return Powf(
            (gamma + 0.111_572_2_f32) / 1.111_572_1_f32,
            1.0f32 / 0.45_f32,
        );
    };
    1.0f32
...
pub fn ToLinearSmpte428(gamma: f32) -> f32 {
    Powf(if gamma > 0.0f32 { gamma } else { 0.0f32 }, 2.6_f32) / 0.916_555_3_f32
}

pub fn ToLinearSrgb(v: u16, bit_depth: i32) -> u32 {
    let shift: i32 = 10i32.wrapping_sub(bit_depth);
    if shift > 0i32 {
        return unsafe { kGammaToLinearTabS[(v as i32).wrapping_shl(shift as u32) as usize] };
    };
    FixedPointInterpolation(
        v as i32,
        unsafe { &kGammaToLinearTabS },
        0i32.wrapping_sub(shift),
        0i32,
    )
}
